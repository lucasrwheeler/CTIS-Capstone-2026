  ...
  try {
    ...
    if (profileMatch) {
      ...
      if (method === "GET") {
        ...
        return { statusCode: 200, headers: cors, body: JSON.stringify(result.rows[0] || { professor_name: profName }) };
      }
    ...
    // ─── GET /user/progress ───────────────────────────────────────────────────
    if (method === "GET" && path.endsWith("/user/progress")) {
      const claims = event.requestContext?.authorizer?.claims || {};
      ...
      const result = await pool.query(
        "SELECT courses, programs, intern_credits FROM user_progress WHERE email = $1",
        [email]
      );
      const row = result.rows[0] || { courses: [], programs: [], intern_credits: {} };
      return { statusCode: 200, headers: cors, body: JSON.stringify(row) };
    }

    // ─── POST /user/progress ──────────────────────────────────────────────────
    if (method === "POST" && path.endsWith("/user/progress")) {
      const claims = event.requestContext?.authorizer?.claims || {};
      const email  = claims.email || "";
      if (!email)
        return { statusCode: 401, headers: cors, body: JSON.stringify({ error: "Unauthorized" }) };
      ...
      const body = typeof event.body === "string" ? JSON.parse(event.body) : (event.body || {});
      const { courses = [], programs = [], intern_credits = {} } = body;

      await pool.query(`
        INSERT INTO user_progress (email, courses, programs, intern_credits, updated_at)
        VALUES ($1, $2, $3, $4, NOW())
        ON CONFLICT (email) DO UPDATE SET
          courses        = EXCLUDED.courses,
          programs       = EXCLUDED.programs,
          intern_credits = EXCLUDED.intern_credits,
          updated_at     = NOW()
      `, [email, JSON.stringify(courses), JSON.stringify(programs), JSON.stringify(intern_credits)]);
